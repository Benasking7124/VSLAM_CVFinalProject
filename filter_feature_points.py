...
def FilterFeaturePoints(left_boxes, right_boxes, feature_points, use_kmeans, num_clusters, depth_threshold):

    # If Feature Points are Filtered by K-Means
    if use_kmeans:
    ...
    # If Feature Points are Filtered using Depth Threshold
    else:
        ...
        for ind in range(feature_points.num_fp):
            ...
            # Append into Dynamic Feature Point if Lesser
            if feature_points.depth[ind] < depth_threshold:
                
                dynamic_feature_points.left_pts = np.vstack([dynamic_feature_points.left_pts, feature_points.left_pts[ind]])
                dynamic_feature_points.left_descriptors = np.vstack([dynamic_feature_points.left_descriptors, feature_points.left_descriptors[ind]])
                dynamic_feature_points.right_pts = np.vstack([dynamic_feature_points.right_pts, feature_points.right_pts[ind]])
                dynamic_feature_points.right_descriptors = np.vstack([dynamic_feature_points.right_descriptors, feature_points.right_descriptors[ind]])
                dynamic_feature_points.disparity = np.vstack([dynamic_feature_points.disparity, feature_points.disparity[ind]])
                dynamic_feature_points.depth = np.vstack([dynamic_feature_points.depth, feature_points.depth[ind]])
                dynamic_feature_points.pt3ds = np.vstack([dynamic_feature_points.pt3ds, feature_points.pt3ds[ind]])
                
            # Append into Static Feature Point if Greater
            else:
                
                static_feature_points.left_pts = np.vstack([static_feature_points.left_pts, feature_points.left_pts[ind]])
                static_feature_points.left_descriptors = np.vstack([static_feature_points.left_descriptors, feature_points.left_descriptors[ind]])
                static_feature_points.right_pts = np.vstack([static_feature_points.right_pts, feature_points.right_pts[ind]])
                static_feature_points.right_descriptors = np.vstack([static_feature_points.right_descriptors, feature_points.right_descriptors[ind]])
                static_feature_points.disparity = np.vstack([static_feature_points.disparity, feature_points.disparity[ind]])
                static_feature_points.depth = np.vstack([static_feature_points.depth, feature_points.depth[ind]])
                static_feature_points.pt3ds = np.vstack([static_feature_points.pt3ds, feature_points.pt3ds[ind]])
